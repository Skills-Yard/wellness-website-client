...
/**
 * Home-page carousel driven strictly by CAROUSEL_VIDEO/CAROUSEL_BANNER
 * campaigns — kept separate from Inspotlight (SPOTLIGHT only) and WallPanel
 * (HIGHLIGHT_VIDEO/HIGHLIGHT_BANNER only) so each campaign type maps to
 * exactly one place on the page.
 */
export default function PromoCarousel({ campaigns, categories }: PromoCarouselProps) {
  ...
  if (carouselCampaigns.length === 0) return null;
  ...
  return (
    <section className="w-full max-w-7xl mx-auto select-none overflow-hidden bg-white py-7 sm:px-6 sm:py-12 md:px-8 md:py-16 lg:px-8 lg:py-20">
      <div className="relative mb-5 flex justify-between gap-4 px-5 sm:mb-8 sm:flex-row sm:items-end sm:gap-6 sm:px-0 md:mb-10">
        <div className="flex-1 pr-16 sm:pr-0">
          ...
          <p className="mt-2 text-xs font-medium leading-relaxed text-gray-600 max-sm:hidden sm:mt-2.5 sm:text-sm md:text-base">
            Fresh picks and seasonal offers, curated just for you
          </p>
        </div>
      </div>
      ...
      <Swiper
        modules={[Navigation]}
        loop={carouselCampaigns.length > 1}
        breakpoints={{
          ...
          return (
            <SwiperSlide key={campaign.id} className="h-auto">
              <article className="relative flex h-[202px] overflow-hidden rounded-[12px] bg-stone-900 shadow-none sm:h-55">
                {campaign.cdnUrl && campaign.type === "CAROUSEL_VIDEO" ? (
                  <CampaignVideo
                    src={campaign.cdnUrl}
