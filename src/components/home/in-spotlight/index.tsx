  ...
  return (
    <section className="w-full py-8 sm:py-12 md:py-16 lg:py-20 px-0 sm:px-6 md:px-8 lg:px-8 max-w-7xl mx-auto select-none overflow-hidden border-y border-stone-100 bg-stone-50/40">
      ...
      <div className="flex relative sm:flex-row sm:items-end justify-between gap-4 sm:gap-6 px-4 sm:px-0 mb-6 sm:mb-8 md:mb-10">
        ...
        {/* Slider Controls */}
        <div className="absolute sm:relative right-4 sm:right-0 top-1 sm:top-0 flex items-center gap-2 select-none flex-shrink-0">
          <button
            type="button"
            ...
            return (
              <SwiperSlide key={card.id} className="h-auto">
                <div className="group cursor-pointer select-none h-full px-2 sm:px-0">
                  {/* Card Container */}
                  <div
                    className={`relative border-none h-[200px] sm:h-[220px] md:h-[220px] w-full overflow-hidden rounded-xl transition-all duration-300 shadow-sm hover:shadow-md flex ${card.bg}`}
                  >
                    {/* Floating Custom Badge */}
                    {card.badge && (
                      <div className={`absolute left-4 top-3.5 z-10 rounded-full px-3 py-0.5 text-[9px] sm:text-[10px] font-bold tracking-wider uppercase ${card.isDarkText
                      ...
                      <div className="relative z-10 flex w-full md:w-[58%] h-full flex-col justify-between p-5 sm:p-6 pt-12 sm:pt-14 md:pt-12">
                        {card.isSpecial ? (
                          ...
                          <div className="space-y-1 sm:space-y-1.5">
                            <h3 className={`text-[15px] sm:text-[17px] md:text-lg font-bold leading-snug tracking-tight line-clamp-2 ${card.isDarkText ? "text-white md:text-gray-900" : "text-white"}`}>
                              {card.title}
                            </h3>
                            <p className={`text-[11px] sm:text-[12px] md:text-[13px] leading-relaxed line-clamp-2 ${card.isDarkText ? "text-white/80 md:text-gray-500" : "text-white/85"}`}>
                              {card.subtitle}
                            </p>
                          </div>
                        )}
                        ...
                        {card.isSpecial ? (
                          ...
                          >
                            {card.cta} <span className="text-[9px] font-bold">&gt;</span>
                          </Button>
                        ) : (
                          <Button
                            size="sm"
