...
import { ChevronDown, ArrowLeft, Search, PhoneCall, MessageSquare } from "lucide-react";

// --- DUMMY DATA (Normally imported from your authData.ts) ---
const professions = ["TV Repair", "AC Service", "Plumber", "Electrician", "Cleaning"];
const cities = ["Delhi NCR", "Banda", "Garhwa", "Surianwan", "Guwahati", "Etah", "Burhanpur", "Warangal", "Nagercoil", "Bhubaneswar"];
...
export default function AuthModal({ onClose }: { onClose: () => void }) {
  ...
  const filteredProfessions = professions.filter(p => p.toLowerCase().includes(workSearch.toLowerCase()));
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 backdrop-blur-sm sm:items-center sm:p-4 animate-in fade-in duration-300">
      ...
      >
        ...
        {showCitySelect && step === "ONBOARDING" ? (
          <div className="absolute inset-0 z-20 flex flex-col bg-white h-full slide-in-from-right-full duration-300">
            ...
            <div className="px-6 pb-4">
              <h2 className="text-2xl font-bold text-black mb-6">Where do you live?</h2>
            ...
            <div className="flex-1 overflow-y-auto px-6">
              {filteredCities.map((c, idx) => (
                <label key={idx} className="flex items-center justify-between py-4 border-b border-dashed border-slate-200 cursor-pointer">
                  <span className="text-lg text-slate-800">{c}</span>
                  <div className={`h-6 w-6 rounded-full border-2 flex items-center justify-center ${city === c ? 'border-blue-600' : 'border-slate-300'}`}>
                    {city === c && <div className="h-3 w-3 rounded-full bg-blue-600" />}
                  </div>
                  {/* Hidden radio for accessibility */}
                  <input type="radio" name="city" value={c} checked={city === c} onChange={() => { setCity(c); setShowCitySelect(false); }} className="hidden" />
                </label>
              ))}
            </div>
          </div>
        ...
        {showWorkSelect && step === "ONBOARDING" ? (
          <div className="absolute inset-0 z-20 flex flex-col bg-white h-full slide-in-from-right-full duration-300">
            ...
            <div className="px-6 pb-4">
              <h2 className="text-2xl font-bold text-black mb-6">What work do you do?</h2>
            ...
            <div className="flex-1 overflow-y-auto px-6">
              {filteredProfessions.map((p, idx) => (
                <label key={idx} className="flex items-center justify-between py-4 border-b border-dashed border-slate-200 cursor-pointer">
                  ...
                  <div className={`h-6 w-6 rounded-full border-2 flex items-center justify-center ${profession === p ? 'border-blue-600' : 'border-slate-300'}`}>
                    {profession === p && <div className="h-3 w-3 rounded-full bg-blue-600" />}
                  </div>
                  {/* Hidden radio for accessibility */}
                  <input type="radio" name="profession" value={p} checked={profession === p} onChange={() => { setProfession(p); setShowWorkSelect(false); }} className="hidden" />
                </label>
              ))}
            </div>
          </div>
          ...
          {step === "PHONE" && (
            <div className="flex flex-col h-full px-6 py-6">
              <div className="flex justify-end mb-6">
                <button onClick={handleSkip} className="rounded-xl border border-slate-200 px-5 py-2 text-sm font-semibold text-slate-700 active:scale-95 transition-transform">
                  Skip
                </button>
              </div>
              ...
              <div className="flex-1 flex flex-col">
                ...
                <p className="mt-2 text-base text-slate-500">
                  We'll send you a text with a verification code.<br /> Standard tariff may apply.
                </p>
                ...
                <div className="mt-8 flex rounded-xl border border-slate-300 focus-within:border-black focus-within:ring-1 focus-within:ring-black overflow-hidden transition-all">
                  <div className="flex items-center gap-1 border-r border-slate-300 px-4 bg-transparent">
                    <span className="text-base text-slate-800 font-medium">+91</span>
              ...
              <div className="mt-auto pt-6 pb-2 text-center">
                <p className="text-sm text-slate-600 mb-4">
                  By continuing, you agree to our <span className="underline font-medium cursor-pointer">T&C</span> and <span className="underline font-medium cursor-pointer">Privacy policy</span>
                </p>
                <button
                  onClick={handlePhoneSubmit}
                  disabled={phone.length < 10}
                  className={`w-full rounded-xl py-4 font-bold transition-all active:scale-[0.98] ${
                    phone.length >= 10 ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-400"
                  }`}
                >
                  Continue
                </button>
          ...
          {step === "OTP" && (
            <div className="flex flex-col h-full px-6 py-6">
              ...
              <div className="flex-1 flex flex-col">
                ...
                <p className="mt-2 text-base text-slate-500">
                  A 6-digit verification code has been sent on<br /> +91 {phone}
                </p>
                  ...
                  {otp.map((digit, index) => (
                    <div
                      ...
                      className={`flex h-12 w-11 sm:h-14 sm:w-12 items-center justify-center rounded-xl border text-xl font-medium transition-all ${
                        digit !== "" ? "border-slate-800 text-slate-900" : index === 0 ? "border-blue-600 shadow-[0_0_0_1px_rgba(37,99,235,1)]" : "border-slate-300"
                      }`}
                    >
                      {digit || (index === 0 ? <span className="animate-pulse">|</span> : "")}
                    </div>
                  ))}
                </div>
          ...
          {step === "ONBOARDING" && (
            <div className="flex flex-col h-full bg-slate-50/50">
              ...
              <div className="px-6 pt-6 pb-8 flex-1 flex flex-col">
                <h2 className="text-3xl font-extrabold text-slate-900 mb-8">Tell us about yourself!</h2>

                <div className="space-y-6 flex-1">
                  {/* Name Input */}
                  <div>
                    <label className="text-base font-medium text-slate-800 ml-1">What's your name?</label>
                    <div className={`mt-2 rounded-full border px-5 py-3.5 bg-white transition-all ${name ? 'border-blue-600' : 'border-slate-300'}`}>
                      <input
                        type="text"
                        value={name}
                  ...
                  <div>
                    <label className="text-base font-medium text-slate-800 ml-1">What work do you do?</label>
                    <div 
                    ...
                    >
                      <span className={`text-base ${profession ? 'text-slate-900' : 'text-slate-400'}`}>
                  ...
                  {/* City Select Trigger */}
                  <div>
                    <label className="text-base font-medium text-slate-800 ml-1">Where do you live?</label>
                    <div 
                      onClick={() => setShowCitySelect(true)}
                      className="mt-2 flex items-center justify-between rounded-full border border-slate-300 bg-white px-5 py-3.5 cursor-pointer hover:border-blue-600 transition-all"
                    >
                      <span className={`text-base ${city ? 'text-slate-900' : 'text-slate-400'}`}>
                ...
                <div className="mt-8 flex flex-col gap-4">
                  <label className="flex items-start gap-3 rounded-xl bg-slate-100 p-4 cursor-pointer">
                    ...
                    <p className="text-sm text-slate-700 leading-tight">
                      ...
                      <span className="font-bold underline cursor-pointer">Terms & conditions</span> and <span className="font-bold underline cursor-pointer">Privacy policy</span>
                    </p>
                  </label>
                  ...
                  <button
                    ...
                    className={`w-full rounded-full py-4 font-bold text-lg transition-all active:scale-[0.98] ${
                      isFormValid ? "bg-blue-600 text-white shadow-md" : "bg-slate-200 text-slate-400"
                    }`}
                  >
                    Continue
                  </button>
      ...
      <div
        className={`fixed left-1/2 z-[60] w-[90%] max-w-sm -translate-x-1/2 rounded-2xl bg-slate-800 p-4 text-white shadow-2xl transition-all duration-500 ${
          notification.visible ? "bottom-6 sm:bottom-10 opacity-100" : "-bottom-24 opacity-0"
        }`}
      >
